...
def attempt_git_push(file_path, msg):
    if not GITHUB_TOKEN or not GITHUB_REPO: return False, "GitHub not set"
    try:
        ...
        payload = {"message":msg,"content":b64,"branch":"main","committer":{"name":"streamlit-bot","email":"bot@example.com"}}
        if sha: payload["sha"] = sha
        resp = requests.put(url, headers=h, json=payload)
        return resp.status_code in [200,201], ("Success" if resp.ok else resp.json().get("message","Failed"))
...
if mode == "Upload New Data":
    ...
    if uploaded:
        try: df = pd.read_excel(uploaded); df.columns = df.columns.str.strip()
        ...
        if confirm and st.button("Save"):
            ...
            low = disp[disp["Production for the Day"] < alert_threshold]
            if not low.empty:
                st.warning("Below threshold:")
                for _,r in low.iterrows(): st.write(f"- {r['Plant']}: {r['Production for the Day']:.1f}")
            c1,c2 = st.columns(2)
            with c1: st.plotly_chart(px.pie(disp, names="Plant", values="Production for the Day", color_discrete_sequence=theme_colors, title="Share"), use_container_width=True)
            with c2: st.plotly_chart(bar_chart(disp, "Production for the Day", theme_colors, "Daily"), use_container_width=True)
            st.plotly_chart(bar_chart(disp, "Accumulative Production", theme_colors, "Accumulative"), use_container_width=True)
...
# ========================================
    ...
    sel_date = st.date_input("Select Date", datetime.strptime(dates[0], "%Y-%m-%d").date())
    ...
    st.plotly_chart(bar_chart(df, "Accumulative Production", theme_colors, "Accumulative Production"), use_container_width=True)
