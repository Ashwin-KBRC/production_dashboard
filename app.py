...
def get_theme_colors(theme_name):
    ...
    themes["Sunset Glow"] = ["#FF5733", "#FFC300", "#FF8C00", "#FF4500", "#C70039"]
    themes["Forest Green"] = ["#228B22", "#32CD32", "#556B2F", "#808000", "#006400"]
    themes["Ocean Breeze"] = ["#00BFFF", "#1E90FF", "#4169E1", "#4682B4", "#5F9EA0"]
    themes["Lavender Dream"] = ["#E6E6FA", "#D8BFD8", "#DDA0DD", "#EE82EE", "#DA70D6"]
...
def apply_chart_theme(fig, x_axis_title="Date Range"):
    ...
    if dark:
        fig.update_layout(plot_bgcolor="#000000", paper_bgcolor="#0f172a")
...
theme_list.append("Sunset Glow")
theme_list.append("Forest Green")
theme_list.append("Ocean Breeze")
theme_list.append("Lavender Dream")
...
if mode == "Analytics":
    ...
    st.markdown("### Top 3 Plants by Accumulative Production")
    accumulative_by_plant = df_filtered.groupby('Plant')['Accumulative Production'].max().sort_values(ascending=False).head(3)
    st.dataframe(accumulative_by_plant, use_container_width=True)
    st.markdown("### Average Production per Plant")
    average_by_plant = df_filtered.groupby('Plant')['Production for the Day'].mean().sort_values(ascending=False)
    st.dataframe(average_by_plant, use_container_width=True)
# ========================================
...
# ========================================
    ...
    st.markdown("### Additional Charts for Production for the Day")
    st.markdown("#### Treemap for Production for the Day")
    fig_treemap_daily = px.treemap(df, path=['Plant'], values='Production for the Day', color='Production for the Day', color_continuous_scale=current_theme_colors)
    st.plotly_chart(apply_chart_theme(fig_treemap_daily), use_container_width=True)
    st.markdown("#### Scatter Plot for Production for the Day")
    fig_scatter_daily = px.scatter(df, x='Plant', y='Production for the Day', color='Plant', size='Production for the Day', color_discrete_sequence=current_theme_colors)
    st.plotly_chart(apply_chart_theme(fig_scatter_daily), use_container_width=True)
    st.markdown("#### Area Chart for Production for the Day")
    fig_area_daily = px.area(df, x='Plant', y='Production for the Day', color='Plant', color_discrete_sequence=current_theme_colors)
    st.plotly_chart(apply_chart_theme(fig_area_daily), use_container_width=True)
    st.markdown("#### Box Plot for Production for the Day")
    fig_box_daily = px.box(df, x='Plant', y='Production for the Day', color='Plant', color_discrete_sequence=current_theme_colors)
    st.plotly_chart(apply_chart_theme(fig_box_daily), use_container_width=True)
    st.markdown("### Additional Charts for Accumulative Production")
    st.markdown("#### Bar Chart for Accumulative Production")
    fig_bar_acc = px.bar(df, x='Plant', y='Accumulative Production', color='Plant', color_discrete_sequence=current_theme_colors)
    st.plotly_chart(apply_chart_theme(fig_bar_acc), use_container_width=True)
    st.markdown("#### Pie Chart for Accumulative Production")
    fig_pie_acc = px.pie(df, names='Plant', values='Accumulative Production', color_discrete_sequence=current_theme_colors)
    st.plotly_chart(fig_pie_acc, use_container_width=True)
    st.markdown("#### Line Chart for Accumulative Production")
    fig_line_acc = px.line(df, x='Plant', y='Accumulative Production', color='Plant', color_discrete_sequence=current_theme_colors)
    st.plotly_chart(apply_chart_theme(fig_line_acc), use_container_width=True)
    st.markdown("#### Violin Plot for Accumulative Production")
    fig_violin_acc = px.violin(df, x='Plant', y='Accumulative Production', color='Plant', color_discrete_sequence=current_theme_colors)
    st.plotly_chart(apply_chart_theme(fig_violin_acc), use_container_width=True)
    hist_frames = []
    for h_d in saved:
        if datetime.strptime(h_d, "%Y-%m-%d").date() <= sel_d:
            try:
                hist_df = load_saved(h_d)
                hist_df['Date'] = pd.to_datetime(hist_df['Date'])
                hist_df = hist_df[~hist_df['Plant'].astype(str).str.upper().str.contains("TOTAL")]
                hist_frames.append(hist_df)
            except: continue
    if hist_frames:
        hist_full_df = pd.concat(hist_frames, ignore_index=True)
        hist_df_filtered = hist_full_df[hist_full_df['Date'] <= pd.to_datetime(sel_d)].copy().sort_values('Date')
        hist_df_filtered = safe_numeric(hist_df_filtered)
        hist_df_filtered = hist_df_filtered.drop_duplicates(subset=['Date', 'Plant'], keep='last')
        st.markdown("### Historical Trend Charts Up to Selected Date")
        st.markdown("#### Trend Line for Production for the Day")
        fig_trend_daily = px.line(hist_df_filtered, x='Date', y='Production for the Day', color='Plant', color_discrete_sequence=current_theme_colors)
        st.plotly_chart(apply_chart_theme(fig_trend_daily), use_container_width=True)
        st.markdown("#### Trend Line for Accumulative Production")
        fig_trend_acc = px.line(hist_df_filtered, x='Date', y='Accumulative Production', color='Plant', color_discrete_sequence=current_theme_colors)
        st.plotly_chart(apply_chart_theme(fig_trend_acc), use_container_width=True)
    st.markdown("### Top 3 Plants by Accumulative Production")
    accumulative_by_plant_hist = df.groupby('Plant')['Accumulative Production'].max().sort_values(ascending=False).head(3)
    st.dataframe(accumulative_by_plant_hist, use_container_width=True)
    st.markdown("### Average Production per Plant")
    average_by_plant_hist = df.groupby('Plant')['Production for the Day'].mean().sort_values(ascending=False)
    st.dataframe(average_by_plant_hist, use_container_width=True)
# ========================================
