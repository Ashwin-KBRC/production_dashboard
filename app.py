...
st.markdown("""
<style>
    /* Hide footer, menu, deploy button, badge */
...
# ========================================
    ...
    if len(saved) < 2:
        ...
        if filtered_df.empty:
            ...
            total_acc_latest = filtered_df.groupby('Plant')['Accumulative Production'].last().sum()
            ...
            ">
                <h1 style="margin:0; font-size:52px; letter-spacing:1px;">TOTAL PRODUCTION</h1>
                <h2 style="margin:15px 0 8px; font-size:48px;">{total_daily_all:,.0f} m³</h2>
                <p style="margin:0; font-size:22px; opacity:0.95;">Daily Production (All Plants Combined)</p>
                <hr style="border:1px solid rgba(255,255,255,0.4); width:60%; margin:25px auto;">
                <h2 style="margin:8px 0 0; font-size:40px;">{total_acc_latest:,.0f} m³</h2>
                <p style="margin:5px 0 0; font-size:20px; opacity:0.9;">Latest Accumulative Total</p>
            </div>
            """, unsafe_allow_html=True)
