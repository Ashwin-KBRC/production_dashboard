...
def generate_smart_insights(df):
    ...
    top_plant = df.groupby('Plant')['Production for the Day'].sum().idxmax()
    top_val = df.groupby('Plant')['Production for the Day'].sum().max()
...
# ========================================
    ...
    if not logs.empty:
        ...
        st.download_button("Export This Log", daily_logs.to_csv(index=False).encode(), "logs.csv", "text/csv")
        st.download_button("Export Full History", logs.to_csv(index=False).encode(), "full_history.csv", "text/csv")
    else:
