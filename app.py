...
if mode == "Upload":
    ...
    if file:
        ...
        if ok and st.button("Save"):
            ...
            st.plotly_chart(px.bar(disp, x="Plant", y="Accumulative Production", color="Plant", color_discrete_sequence=theme_colors), use_container_width=True)
...
# ========================================
    ...
    st.plotly_chart(px.bar(df, x="Plant", y="Accumulative Production", color="Plant", color_discrete_sequence=theme_colors), use_container_width=True)
...
# ========================================
    ...
    # DOWNLOAD
    excel = generate_excel_report(debug, f"{start}_to_{end}")
    st.download_button("Download", excel, f"monthly_accumulative.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
