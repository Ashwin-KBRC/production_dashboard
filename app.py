...
<style>
    ...
    .stExpander > div > label { font-weight: bold; color: #1e3a8a; }
</style>
...
# === Color Themes (Professional Palettes) ===
COLOR_THEMES = {
...
def save_csv(df: pd.DataFrame, date_obj: datetime.date, overwrite: bool = False) -> Path:
    ...
    if p.exists() and not overwrite:
        raise FileExistsError(f"{fname} already exists. Check 'overwrite' to replace.")
...
def attempt_git_push(file_path: Path, commit_message: str):
    ...
    try:
        ...
        if r.status_code in [200, 201]:
            return True, f"Backed up to GitHub: {file_path.name}"
        else:
...
def bar_chart(df, col, colors, title):
    ...
    fig.update_traces(texttemplate="%{text:.2s}", textposition="outside", textfont_size=14)
...
def aggregated_bar_chart(df, val_col, group_col, colors, title):
    ...
    unique_groups = agg[group_col].unique()
...
def date_range_selector(key_prefix: str, default_start=None, default_end=None):
    with st.expander("Date Range Filter", expanded=True):
        c1, c2, c3 = st.columns([3, 3, 1])
        with c1:
            start = st.date_input("Start Date", value=default_start or (datetime.today() - timedelta(days=30)), key=f"{key_prefix}_start")
        with c2:
            end = st.date_input("End Date", value=default_end or datetime.today(), key=f"{key_prefix}_end")
        with c3:
        ...
        if missing:
            ...
            if confirm and st.button("Upload & Save"):
                ...
                try:
                    ...
                    pushed, msg = attempt_git_push(saved_path, f"Add production data for {selected_date:%Y-%m-%d}")
                ...
                st.markdown(f"### Totals\n- **Daily:** {total_daily:,.2f} m³\n- **Accumulative:** {total_acc:,.2f} m³")
                ...
                with col1: st.plotly_chart(pie_chart(df_disp, "Production for the Day", theme_colors, "Production Share"), use_container_width=True)
                with col2: st.plotly_chart(bar_chart(df_disp, "Production for the Day", theme_colors, "Production per Plant"), use_container_width=True)
                st.plotly_chart(area_chart(df_disp, "Production for the Day", theme_colors, "Production Flow"), use_container_width=True)
                st.plotly_chart(bar_chart(df_disp, "Accumulative Production", theme_colors, "Accumulative Production"), use_container_width=True)

                top_plant = df_disp.loc[df_disp["Production for the Day"].idxmax()]
                st.success(f"Top Producer: **{top_plant['Plant']}** — {top_plant['Production for the Day']:,.1f} m³")

                excel_file = generate_excel_report(df_disp, selected_date.strftime("%Y-%m-%d"))
                st.download_button("Download Excel Report", excel_file, f"production_{selected_date:%Y-%m-%d}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
# === VIEW HISTORICAL DATA ===
    ...
    if not dates:
        ...
        start, end, _ = date_range_selector("hist")
...
# === MANAGE DATA ===
    ...
    if not dates:
        ...
        if action == "Rename":
            ...
            st.warning("This will permanently delete the file.")
...
# === ANALYTICS ===
    ...
    st.header("Multi-Day Analytics & Trends")
    dates = list_saved_dates()
    if len(dates) < 2:
        st.info("Upload at least 2 days of data to view analytics.")
    else:
        start, end, _ = date_range_selector("analytics")
        ...
        if not frames:
            ...
            st.markdown("### Weekly Total Production per Plant")
            ...
            st.markdown("### Monthly Total Production per Plant")
            ...
            st.markdown("### Plant Production Trend Over Time")
            ...
            st.download_button("Download Full Report", excel, f"analytics_{start}_to_{end}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
