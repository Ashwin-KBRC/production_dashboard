...
# ========================================
# FORCE LIGHT MODE — NO DARK BACKGROUND
# ========================================
st.set_page_config(
...
# Force clean white background and light theme
st.markdown("""
<style>
    /* Main app background */
    .stApp { 
    ...
    .css-1d391kg, .css-1y0t9e2, .css-1cpxl2t {
        background-color: #ffffff !important;
    }
    /* Text color fix */
...
# ========================================
# YOUR ORIGINAL CODE — 100% UNCHANGED BELOW
# ========================================
DATA_DIR = Path("data")
DATA_DIR.mkdir(parents=True, exist_ok=True)
REQUIRED_COLS = ["Plant", "Production for the Day", "Accumulative Production"]
...
def safe_numeric(df: pd.DataFrame) -> pd.DataFrame:
    df2 = df.copy()
    df2["Production for the Day"] = pd.to_numeric(df2[" GlobalProduction for the Day"], errors="coerce").fillna(0.0)
    df2["Accumulative Production"] = pd.to_numeric(df2["Accumulative Production"], errors="coerce")
    df2["Accumulative Production"] = df2["Accumulative Production"].fillna(method='ffill').fillna(0)
    return df2
        ...
        if filtered_df.empty:
            ...
            for idx in range(min(3, len(top_daily))):
                ...
                with podium_cols[idx]:
                    st.markdown(
                        f"<div class='podium-card' style='border-color:{color};'>"
                        f"<div class='rank-line' style='border-color:{color}; color:{color}; font-size:28px;'>**{rank}**</div>"
                        f"<div style='font-size:28px; font-weight:bold; color:#1a1a1a; margin:10px 0;'>{row['Plant']}</div>"
                        f"<div style='font-size:28px; color:#1a1a1a;'>{row['Monthly Daily Total']:,.1f} m³</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            for idx in range(min(3, len(top_acc))):
                ...
                with podium_cols_acc[idx]:
                    st.markdown(
                        ...
                        f"<div class='rank-line' style='border-color:{color}; color:{color}; font-size:28px;'>**{rank}**</div>"
                        f"<div style='font-size:28px; font-weight:bold; color:#1a1a1a; margin:10px 0;'>{row['Plant']}</div>"
                        f"<div style='font-size:28px; color:#1a1a1a;'>{row['Monthly Accumulative']:,.1f} m³</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
            st.markdown("**Note**: Each week/month has **unique gradient colors**. KABD is **bold red**.")
