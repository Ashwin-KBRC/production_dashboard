...
# ========================================
# COMPLETELY REMOVE COLLAPSE BUTTON & HAMBURGER MENU
# ========================================
st.markdown("""
<style>
...
# ========================================
# PLOT HELPERS (exactly as you had)
# ========================================
...
def bar_chart(df: pd.DataFrame, value_col: str, colors: list, title: str):
    ...
    fig.update_traces(texttemplate="%{text:,.1f}", textposition="outside", textfont=dict(size=16, color="black", family="Arial"), cliponaxis=False)
    fig.update_layout(title_font=dict(size=18), margin=dict(t=60, b=280, l=60, r=40), xaxis_tickangle=0, xaxis_gridcolor="#E0E0E0", yaxis_gridcolor="#E0E0E0")
    return fig

def line_chart(df: pd.DataFrame, value_col: str, colors: list, title: str):
    df = df.copy()
    df[value_col] = pd.to_numeric(df[value_col], errors='coerce').fillna(0)
    fig = px.line(df, x="Plant", y=value_col, markers=True, title=title, color_discrete_sequence=colors, text=df[value_col].round(1))
    fig.update_traces(marker=dict(size=10, line=dict(width=2, color="DarkSlateGrey")), line=dict(width=3), textposition="top center", texttemplate="%{text:,.1f}")
    fig.update_layout(title_font=dict(size=18), margin=dict(t=60, b=40, l=60, r=40), xaxis_gridcolor="#E0E0E0", yaxis_gridcolor="#E0E0E0")
    return fig
...
def aggregated_bar_chart(df: pd.DataFrame, value_col: str, group_col: str, base_colors: list, title: str):
    ...
    for trace in fig.data:
        ...
        if group_key not in palette_map: continue
...
with st.sidebar:
    ...
    theme_choice = st.selectbox("Theme", list(COLOR_THEMES.keys()),
        index=list(COLOR_THEMES.keys()).index(st.session_state.theme))
    if theme_choice != st.session_state.theme:
        st.session_state.theme = theme_choice
        st.rerun()

    alert_threshold = st.number_input("Alert threshold (m³)", min_value=0.0, value=50.0, step=0.5)

    st.markdown("---")
    st.caption("Upload Excel with columns: Plant, Production for the Day, Accumulative Production")
    st.caption("Set `GITHUB_TOKEN` & `GITHUB_REPO` in secrets for auto-push")

theme_colors = COLOR_THEMES[st.session_state.theme]
st.title("PRODUCTION DASHBOARD")

# ========================================
# MODES (100% your original code)
# ========================================
if mode == "Upload New Data":
    ...
    if uploaded:
        ...
        if missing:
            ...
            if confirm and st.button("Upload & Save"):
                ...
                st.write("GitHub:" if pushed else "GitHub (failed):", message)
                ...
                acc_df = df_display[["Plant", "Accumulative Production"]].copy().sort_values("Accumulative Production", ascending=False)
                ...
                st.download_button("Download Excel", excel_file, f"report_{selected_date:%Y-%m-%d}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if not saved_list:
        ...
        acc_hist = df_hist_disp[["Plant", "Accumulative Production"]].copy().sort_values("Accumulative Production", ascending=False)
        ...
        for date_str in saved_list:
            ...
            with col3:
                if st.button("Download", key=f"dl_{date_str}"):
                    ...
                    st.download_button("Download XLSX", excel, f"{date_str}.xlsx", key=f"dlbtn_{date_str}")
    ...
    if len(saved) < 2:
        ...
        if filtered_df.empty:
            ...
            <div style="background: linear-gradient(135deg, #1e40af, #3b82f6); color: white; padding: 70px; border-radius: 40px; text-align: center; margin: 40px 0; box-shadow: 0 25px 60px rgba(0,0,0,0.45); font-family: 'Arial Black', sans-serif;">
                ...
                <p style="margin:0; font-size:32px;">{start_date.strftime('%b %d')} → {end_date.strftime('%b %d, %Y')} • All Plants</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with colA:
                ...
                for i, row in top_avg.iterrows():
                    rank, color = ["1st", "2nd", "3rd"][i], ["#FFD700", "#C0C0C0", "#CD7F32"][i]
                    st.markdown(f"""
            ...
            with colB:
                ...
                for i, row in top_acc.iterrows():
                    rank, color = ["1st", "2nd", "3rd"][i], ["#1E90FF", "#4682B4", "#5F9EA0"][i]
                    st.markdown(f"""
                    <div style="background:white;padding:30px;border-radius:20px;margin:15px 0;border-left:12px solid {color};box-shadow:0 10px 25px rgba(0,0,0,0.15);">
                        <h3 style="margin:0;color:{color}">{rank} • {row['Plant']}</h3>
